...
- The client sends the two results of a game at the same time. It does not wait for the first response.
- The server writes one result at a time in each process. The `results` action holds a lock (`RESULTS_LOCK`) around the find-or-create and the counter increment. Two requests that arrive at the same time both return 200 in less than 1 second. No request waits for the SQLite busy timeout. This is true for two new names, for the same new name, and for a new name and a known name.
- If two requests for the same new name arrive at the same time, the server makes one row, and both results count. If another process creates the name between the find and the insert, the server rescues `ActiveRecord::RecordNotUnique` or `ActiveRecord::RecordInvalid` and finds the player again.
