...
- `PUT/PATCH /api/v1/players/:id` accepts `name`, `wins`, `losses` and `draws`, and persists them. It sets the counters to the values that it gets. It does not add to them.
...
- The client does not use this endpoint to record a result. It uses `POST /api/v1/players/results` (E3-S4). [E6-S7](32-e6-s7-restrict-cors-and-protect-write-endpoints.md) covers the protection of this endpoint.
