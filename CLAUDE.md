...
1. **Branch.** Create `fix/<story-id>-<slug>` from `master`. Take the story id from the first unchecked item in `sdd/TODO.md`, and the slug from the title of its E6 story. Do the next steps on this branch.
2. **Check.** Read the first unchecked item in `sdd/TODO.md`, its story spec, its E6 story and the code. Done when every listed gap is confirmed or refuted with a `file:line`, and every gap that the E6 story covers but the TODO does not list is noted. Also note an item with no E6 story reference, or with only an epic reference such as "(see E6)".
...
4. **Plan.** Base the plan on the new criteria from step 3. Save the plan to `plans/<story-id>-<slug>.md`, for example `plans/e1-s2-spec-update.md`. Done when the file exists.
...
6. **TODO.** Check off the story's items and the E6 story with its criteria. Correct the story references. Add the gaps from step 2 that the TODO did not list, and check them off. Done when each item that this fix closes is checked.
