...
2. **Spec.** Rewrite the acceptance criteria of the story spec so they state the target behavior from the E6 story. Keep the title, the epic lines and the user story. Done when each TODO gap is a violation of one criterion.
3. **Plan.** Base the plan on the new criteria from step 2. Save the plan to `plans/<story-id>-<slug>.md`, for example `plans/e1-s2-spec-update.md`. Done when the file exists.
